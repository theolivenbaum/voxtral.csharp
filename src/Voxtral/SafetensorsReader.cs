...
namespace Voxtral
{
    public class SafetensorsReader : IDisposable
    {
        ...
        public Tensor<float> LoadTensor(string name)
        {
            ...
            // Create Tensor
            // System.Numerics.Tensors.Tensor.Create expects ReadOnlySpan<nint> for lengths.
            // But verify .NET 10 API.
            // Using Tensor.Create(data, lengths)

            nint[] lengths = info.shape.Select(x => (nint)x).ToArray();

            // For now, returning a dense tensor wrapping the array.
            // Note: System.Numerics.Tensors.Tensor<T> is abstract in some versions or concrete?
            // in .NET 10 / 9 Preview, it's a new type.
